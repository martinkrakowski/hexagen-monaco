...
**Status:** Accepted
**Context:** PRs in this repo are reviewed by **CodeRabbit**, **qodo**, and **greptile-apps**. Their comments are high-signal but can be stale (reviewed an earlier commit), misattributed (historical vs. current code), or proportionately deferrable. Disposition each one — never reflexively apply or dismiss.
...
3. **Verify the cited rule.** When a comment invokes a repo convention, check it against the source before complying. Bots infer conventions from surrounding code and get them wrong. Worked example: both CodeRabbit and qodo have flagged mixed `expect()` / `assert.*` as a violation, but **ADR-0044 §2 permits `expect()`** and `AGENTS.md:38` records `assert.*` as retained codebase-wide; the real rule (**ADR-0044 §3**) is _per file_, not "prefer `node:assert`". Fix the underlying inconsistency if there is one, and correct the rule citation in the same reply.
...
7. **Classify severity.** Separate **Blocker** (silent data loss, wrong output, security, a shipped regression) from **Advisory / Backlog** (valid but proportionate to defer). State which, and why a deferral is proportionate + where it's tracked.

   **Verifier defects are their own tier.** When the flagged code is itself a check — a guard, a lint rule, a CI script, a schema — _"no live case in this package today"_ is a far weaker refutation than it is for production code. Production code with no live case does nothing; **a verifier with a blind spot under-reports silently, forever, and is trusted while doing it.** Default to fixing the parser/matcher rather than proving the current tree happens to be clean. If you do refute on "no live case", state that the hole remains and name it.
...
2. **Every guard needs an anti-vacuity assertion and a mutation proof.**
   - _Anti-vacuity:_ assert the scan found a non-zero population **before** asserting that population is clean. Without it the guard goes green the moment its discovery pattern drifts — measured twice in this repo: a drifted pattern plus a neutered floor passed **green over an empty population**, and weakening a floor to `>= 0` turned a red suite green while the defect was still present.
   - _Mutation:_ break the thing the guard guards, confirm it fails, and **quote the failure**. A guard nobody tried to defeat is not evidence.
   - Restore with an **inverse edit plus a sentinel grep**, never `git checkout` — that reverts uncommitted work alongside the probe.
...
5. Re-run the gate (`yarn build && yarn typecheck && yarn lint`, then `yarn test`); note the suite count. **Confirm the check actually ran** — a `pull_request` check does not re-fire when the base branch moves, so a stale green (or stale red) can survive a base that has changed underneath it.
