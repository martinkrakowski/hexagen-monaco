...
import type { PipelineState } from '../../../../domain/value-objects/pipeline-state';
...
describe('ExecuteDomainExtractionUseCase', () => {
  test('happy path: valid stage0 state returns DomainAnalysis', async () => {
    ...
    if (result.success) {
      ...
      assert.deepStrictEqual(result.value.aggregateRoots, [{ name: 'User', subdomain: 'Identity' }]);
    }
  });
  ...
  test('retry path: fails 2 times then succeeds', async () => {
    ...
    const mockPort = createMockLLMPort([errorStream1, errorStream2, successStream]);
    const useCase = new ExecuteDomainExtractionUseCase(mockPort);
    const result = await useCase.execute(mockStage0State);
    assert.strictEqual(result.success, true);
    if (result.success) {
  ...
  test('max retries exceeded: returns StageMaxRetriesError', async () => {
    ...
    const mockPort = createMockLLMPort([errorStream1, errorStream2, errorStream3]);
    const useCase = new ExecuteDomainExtractionUseCase(mockPort);
    const result = await useCase.execute(mockStage0State);
