...
{"name": "drift-analytics"}
    ...
    assert.strictEqual(isLikelyNDJSON, false, "Formatted JSON array should not trigger NDJSON detection");
...
{"name":"NotificationService","type":"core"}`;
    ...
    assert.strictEqual(isLikelyNDJSON, true, "NDJSON with multiple { lines should trigger detection");
  });
});
