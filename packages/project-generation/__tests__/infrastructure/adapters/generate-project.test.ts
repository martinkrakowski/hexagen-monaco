// Minimal test suite for IGenerateProjectPort in the project-generation package.
...
(async () => {
  ...
  const defaultResult = await defaultFake.generate(defaultInput);
  ...
  const customResult = await customFake.generate(customInput);
  assert.deepStrictEqual(
    customResult,
