...
export class FileSystemReportAdapter implements ReportRepositoryPort {
  async save(
    ...
    try {
      ...
      return {
        success: false,
        error: new ReportPersistenceError("save", manifestPath, e as Error),
      };
    }
  }
  ...
  async load(
    ...
    try {
      ...
      return {
        success: false,
        error: new ReportPersistenceError("load", manifestPath, e as Error),
      };
    }
  }

  async appendPhaseReport(
    featureId: FeatureId,
    phase: string,
    content: string,
    projectRoot: string,
    ...
    try {
      ...
      return {
        success: false,
        error: new ReportPersistenceError(
          "appendPhaseReport",
          reportPath,
          e as Error,
        ),
