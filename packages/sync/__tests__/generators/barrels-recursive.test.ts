...
describe("generateRecursiveBarrels", () => {
  ...
  it("5a. empty layer dir gets NO barrel (ADR-0050: empty layer barrels must not come back)", async () => {
    // Before 2026-08-23 the walker special-cased domain/application/
    // infrastructure and wrote `export {};` stubs into them. That emitter
    // outlived ADR-0050 and #554 (no unused layer folders): P4.1 deleted the
    // 20 remaining stubs and `sync` in CI (step "Execute Hardened Sync",
    // which WRITES) recreated four of them — plus the #548 core-domain /
    // runtime deletions, on an untouched main. TS18003 for a module with
    // zero inputs is handled by the package-root stub
    // (package-src-index.ts), so the layer stub has no remaining purpose.
    await createFixture(moduleDir, {
      src: {
    ...
    assert.strictEqual(
      await exists(moduleDir, "src/domain/index.ts"),
      false,
      "an empty layer dir must not receive a stub barrel",
    );
  });

  it("5b. a generated `export {}` stub already present in an empty layer dir is deleted", async () => {
    await createFixture(moduleDir, {
      src: {
        domain: {
    ...
    assert.strictEqual(
      await exists(moduleDir, "src/domain/index.ts"),
      false,
      "a generated stub in an empty layer dir must be removed, not kept",
    );
  });
  ...
  it("5c. a hand-written barrel in an empty layer dir is left alone", async () => {
    ...
    // content. Hand-written files are never the generator's to rewrite.
    const handWritten = "// hand-maintained\nexport type Marker = never;\n";
    await createFixture(moduleDir, {
      src: {
        domain: { "index.ts": handWritten },
      },
    });
    ...
    assert.strictEqual(
      ...
      handWritten,
      "a hand-written barrel must be byte-identical after sync",
    );
  });
  ...
  it("7. a comment-only generated layer barrel is removed, not rewritten to a stub", async () => {
    // Until 2026-08-23 this case was rewritten to `export {};`. Rewriting
    // would recreate exactly the empty layer barrel ADR-0050 retired; the
    // generated marker makes the file the generator's to delete.
    await createFixture(moduleDir, {
      src: {
        domain: {
          "index.ts": "// @generated by @hexagen/sync\n\n// No exports yet\n",
    ...
    assert.strictEqual(
      await exists(moduleDir, "src/domain/index.ts"),
      false,
      "comment-only generated barrel in an empty layer dir must be deleted",
    );
  });
