...
describe("generateRecursiveBarrels", () => {
  ...
  it("6. empty non-layer subdirectory has its generated barrel removed", async () => {
    ...
    //
    // ⚠ LATENT BUG DISCOVERED (reported, NOT fixed — see test deliverable):
    //   The parent barrel is built using `hasBarrel(subDirPath)` at walk time
    //   (recursive.ts:206), which checks the filesystem BEFORE queued writes
    //   run. As a result, the parent barrel still re-exports `unused/` even
    //   though `unused/index.ts` is queued for deletion. After the write
    //   phase, the parent contains a broken reference to a file that no
    //   longer exists on disk.
    //
    //   This test asserts the CURRENT (buggy) observable behavior so the
    //   suite stays green; a follow-up fix should either:
    //     (a) defer the hasBarrel check until after pendingWrites is resolved,
    //         or (b) remove deleted paths from parent export entries at queue
    //         time.
    await createFixture(moduleDir, {
      src: {
        domain: {
    ...
    // Observed-bug assertion: parent still references the now-deleted child.
    // Flip this to `doesNotMatch` after the upstream fix lands.
    assert.match(
      parent,
      /export \* from "\.\/unused\/index\.js";/,
      "LATENT BUG: parent still references deleted subdir (see comment above)",
    );
  });
