...
import { FakeOwnershipRegistryPort } from '../../doubles/ports/ownership-registry.fake';

// Test default empty map
(async () => {
  const fake = new FakeOwnershipRegistryPort();
  const map = await fake.loadOwnershipMap();
  assert.deepStrictEqual(map, { success: true, value: [] }, 'Default map should be empty');
})();
...
(async () => {
  ...
  const map = await fake.loadOwnershipMap();
  assert.deepStrictEqual(map, { success: true, value: customMap }, 'Custom map should be returned');
})();
...
(async () => {
  ...
  assert.deepStrictEqual(result, { success: true, value: undefined }, 'Registration should succeed');
  const map = await fake.loadOwnershipMap();
  assert.deepStrictEqual(map, { success: true, value: [{ portName: 'NewPort', owningPackage: 'PackageC' }] }, 'Map should contain the new registration');
})();
...
(async () => {
  ...
  const map = await fake.loadOwnershipMap();
  assert.deepStrictEqual(map, { success: true, value: [{ portName: 'SharedPort', owningPackage: 'PackageX' }] }, 'Map should remain unchanged after conflict');
})();
...
(async () => {
  ...
  assert.strictEqual(canDeclareOther, false, 'Different package should be denied');
})();
...
(async () => {
  ...
  assert.strictEqual(owner, 'PackageD', 'Should return the correct owning package');
