/**
 * Generic in-memory test double shared by the architectural-enforcement port
 * fakes: `execute` echoes its input unchanged by default, or applies a custom
 * async implementation registered via `setBehavior`. Subclassed per port so the
 * tests read with a port-specific name.
 */
