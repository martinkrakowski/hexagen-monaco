...
export function analyzeManifestCompleteness(
  ...
  const isPopulated = (arr: unknown[] | undefined) => Array.isArray(arr) && arr.length > 0;
  ...
  if (manifest.boundedContexts && manifest.boundedContexts.length > 0) {
    ...
    contextsComplete = manifest.boundedContexts.length > 1 || 
                       firstCtx.name !== emptyFormValues.boundedContexts[0].name ||
                       firstCtx.infrastructureTarget !== emptyFormValues.boundedContexts[0].infrastructureTarget;
  }

  const portsComplete = !!manifest.boundedContexts && manifest.boundedContexts.some(
    ctx => isPopulated(ctx.portConfiguration?.inboundPorts) || isPopulated(ctx.portConfiguration?.outboundPorts)
  );
  
  // Mappings are complete if there are mappings, or if they are not needed (only 1 context)
  const peerMappingsComplete = !!(isPopulated(manifest.peerMappings) || (manifest.boundedContexts && manifest.boundedContexts.length <= 1));
