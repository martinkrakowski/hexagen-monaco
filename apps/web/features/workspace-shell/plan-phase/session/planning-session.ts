 ...
 * The round is recovered from the highest persisted turn round.
 */
export function sessionStateFromLayer(layer: {
  readonly status?: SessionStatus;
  readonly maxRounds?: number;
  readonly turns: ReadonlyArray<{ readonly round?: number }>;
}): PlanningSessionState {
  const maxRounds =
    layer.maxRounds !== undefined && layer.maxRounds > 0
      ? layer.maxRounds
      : DEFAULT_MAX_ROUNDS;
  const round = Math.max(1, ...layer.turns.map((t) => t.round ?? 0));
  ...
  if (status === "awaiting-human") {
    // A stored awaiting-human carries no reason — treat as a plain pause so
    // Resume re-runs the proposer side (safe default: a duplicate proposal is
    // recoverable; a skipped critique verdict is not).
