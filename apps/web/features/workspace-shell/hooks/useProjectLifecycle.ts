...
export function useProjectLifecycle(
  ...
  useEffect(() => {
    if (draftLoading || uiState.kind === "edit") return;
    if (!activeWorkspace || projects.length === 0) return;

    if (hasProjectUrlParam && draft) {
      clearDraft();
      return;
    }
  ...
  const handleLoadProject = useCallback(
    async (id: string) => {
      ...
      // Update URL with project ID so refresh preserves the selection
