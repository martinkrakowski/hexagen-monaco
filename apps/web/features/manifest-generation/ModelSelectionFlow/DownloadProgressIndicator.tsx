...
export function DownloadProgressIndicator({
  ...
  useEffect(() => {
    if (progress > lastProgress) {
      ...
      if (timeElapsed > 500) {
        // Only update calculations every 500ms
    ...
    <div className="flex flex-col space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          {phase === "downloading" ? (
            <>
              Downloading model files from HuggingFace. This may take a while
              depending on your connection.
            </>
          ) : (
            <>
              Setting up the model on your device. This will be faster next
              time.
            </>
          )}
        </p>
      ...
      <div className="flex flex-col space-y-3">
        <div className="flex items-center text-sm">
          <div
            className={`h-4 w-4 mr-2 ${phase === "downloading" ? "text-primary" : "text-muted"}`}
          >
            <Icon name="download" />
          </div>
          <span
        ...
        <div className="flex items-center text-sm">
          <div
            className={`h-4 w-4 mr-2 ${phase === "compiling" ? "text-primary" : "text-muted"}`}
          >
            <Icon name="settings" />
          </div>
          <span
        ...
        <div className="flex items-center text-sm">
          <div
            className={`h-4 w-4 mr-2 ${phase === "loading" ? "text-primary" : "text-muted"}`}
          >
            <Icon name="settings" />
          </div>
          <span
