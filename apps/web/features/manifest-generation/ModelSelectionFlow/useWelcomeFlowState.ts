...
export interface WelcomeFlowActions {
  ...
  selectCloudProvider: (provider: string, apiKey: string, remember: boolean) => void;
  skipAiSetup: () => void;
 ...
 */
export function useWelcomeFlowState(llmContext: LocalLLMContext): [WelcomeFlowState, WelcomeFlowActions] {
  const { 
    engineState, 
    initializeModel, 
    cancelDownload, 
  ...
  const [apiKeyManager, setApiKeyManager] = useState<ApiKeyManager | null>(null);
    ...
    if (!gpuDetection.isLoading) {
      ...
      if (!gpuDetection.isWebGPUSupported && !gpuDetection.isLoading) {
        setFlowState(prev => ({ 
          ...
          errorCode: prev.state === 'idle' ? "webgpu_unavailable" as WelcomeFlowErrorCode : prev.errorCode,
        }));
      }
    }
  ...
  }, [gpuDetection.isLoading, gpuDetection.isWebGPUSupported, gpuDetection.isBrowserSupported, 
      gpuDetection.isHardwareAdequate, gpuDetection.estimatedVRAM, gpuDetection.isRecommended]);
  ...
  useEffect(() => {
    ...
    if (engineState.status === "ready") {
      ...
      const runSmokeTest = async () => {
        try {
          await llmContext.sendGovernanceMessage(
            "Respond with 'OK'",
            "You are a test helper. Reply only with the word OK.",
          );
          setFlowState(prev => ({ ...prev, state: "generating", isModelReady: true }));
        } catch {
          setFlowState(prev => ({ 
            ...prev, 
            state: "error",
            error: "The model cache appears to be corrupted. Please delete and re-download the model.",
      ...
      if (lastModelId) {
        hasModelInCache(lastModelId).then(isInCache => {
          if (isInCache) {
            ...
            if (preferences.current.autoLoadEnabled && engineState.status !== "ready") {
              initializeModel(lastModelId).catch(() => {
                // Silent failure for auto-load attempt
              });
            }
  ...
  }, [engineState.loadedModelId, engineState.status, hasAnyCachedModel, hasModelInCache, initializeModel]);

  const transitionTo = useCallback((state: WelcomeScreenState) => {
    setFlowState(prev => ({ ...prev, state }));
  }, []);

  const selectLocalModel = useCallback((modelId: DomainModelId, remember: boolean) => {
  ...
  const selectCloudProvider = useCallback(async (provider: string, apiKey: string, remember: boolean) => {
    setFlowState(prev => ({ 
      ...prev, 
    ...
    try {
      ...
      setFlowState(prev => ({
        ...prev,
        state: "error",
        error: error instanceof Error ? error.message : "An error occurred validating your API key",
        errorCode: "key_rejected" as WelcomeFlowErrorCode,
      }));
    }
  }, [apiKeyManager, validateApiKey]);

  const loadSavedApiKey = useCallback(async (provider: string): Promise<string | null> => {
  ...
  const setError = useCallback((message: string, errorCode?: WelcomeFlowErrorCode) => {
    setFlowState(prev => ({ 
      ...prev, 
      state: "error",
      error: message,
