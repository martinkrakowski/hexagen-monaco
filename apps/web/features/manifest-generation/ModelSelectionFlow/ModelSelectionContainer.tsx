...
export function ModelSelectionContainer({
  ...
  const handleLocalSelected = useCallback((modelId: DomainModelId, remember: boolean) => {
    flowActions.selectLocalModel(modelId, remember);
  }, [flowActions]);
  
  const handleCloudSelected = useCallback((provider: string, apiKey: string, remember: boolean) => {
  ...
  const renderContent = () => {
    ...
    if (engineState.status === "unsupported_browser" || engineState.status === "no_webgpu") {
    ...
    switch (flowState.state) {
      ...
      case "key_validation":
        return <CloudProviderForm 
          provider={flowState.cloudProvider || "openai"}
          apiKey={flowState.cloudApiKey || ""}
          onSubmit={(provider, key) => handleCloudSelected(provider, key, !!flowState.rememberedChoice)}
