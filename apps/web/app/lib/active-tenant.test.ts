...
describe("active-tenant store", () => {
  ...
  it("storage that throws on read defaults to personal instead of crashing", async () => {
    const getItem = vi
  ...
  it("storage that throws on write still switches in memory and still notifies", async () => {
    const store = await freshStore();
    const setItem = vi
