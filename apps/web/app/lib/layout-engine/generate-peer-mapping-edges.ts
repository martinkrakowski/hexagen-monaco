...
import type { HexagonEdge } from "@hexagen/visualization";
...
interface GeneratePeerMappingEdgesOptions {
  ...
  /** Nodes already generated by earlier phases; used to verify the referenced contexts exist. */
  existingNodes: readonly HexagonNodeWithLayout[];
}

/**
 * Cross-context peer-mapping edges — connects consumer (source)
 * to provider (target) along the East→West axis, routing around
 * hexagon bodies to avoid penetrating them.
 *
 * Each edge is labelled `Consumer → Provider (OHS|ACL)` reflecting
 * the integration pattern. Only emits an edge when BOTH referenced
 * contexts exist in the generated node graph (guards against stale
 * mapping data pointing at deleted contexts).
 */
