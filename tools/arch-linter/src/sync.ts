  ...
  for (const [layerName, folderName] of Object.entries(effectiveLayers)) {
    ...
    if (dryRunMode) {
      ...
      try {
        ...
        logger.error(`Failed to create layer folder ${layerPath}: ${msg}`);
      }
    }

    if (layerName === 'application') {
      const useCasesPath = path.join(layerPath, 'use-cases');
      const portsOutPath = path.join(layerPath, 'ports/out');
      if (dryRunMode) {
        logger.info(
          `[dry-run] Would ensure application subfolders: use-cases, ports/out`
        );
      } else {
        try {
          await fs.mkdir(useCasesPath, { recursive: true });
          await fs.mkdir(portsOutPath, { recursive: true });
          logger.info(`Ensured application subfolders: use-cases, ports/out`);
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          logger.error(`Failed to create application subfolders: ${msg}`);
        }
      }
    }

    if (layerName === 'infrastructure') {
      const adaptersPath = path.join(layerPath, 'adapters');
      if (dryRunMode) {
        logger.info(
        ...
        try {
          await fs.mkdir(adaptersPath, { recursive: true });
          logger.info(`Ensured infrastructure subfolder: adapters`);
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          logger.error(`Failed to create infrastructure subfolder: ${msg}`);
        }
      }
    }
  }
...
async function generateLayerBarrel(
  ...
  // Use safety version (reexportStyle not yet fully utilized; TODO: expand for named exports if needed)
...
// ... (TODO: Implement generateEntityOrValueObjectStub, generateUseCaseStub, generatePortStub, generateStubsForModule)
    ...
    for (const currentModule of manifest.modules) {
      ...
      if (dryRun) {
        ...
        try {
          ...
          const msg = err instanceof Error ? err.message : String(err);
          logger.error(`Failed to create src folder ${srcDir}: ${msg}`);
        }
      }
      ...
      // await generateStubsForModule(moduleDir, currentModule, force, dryRun); // TODO: Implement and uncomment when ready
    }
