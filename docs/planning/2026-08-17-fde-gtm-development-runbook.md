...
5. Note version boundary: already-published tarballs (≤0.10.0) remain under evaluation license forever; relicense applies from next release. State in CHANGELOG.
...
| Fix                                                | Where                                           | Why now                                                  |
| -------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------- |
| Delete stale "ships dark / NOT yet routed" comment | `execute-full-staged-generation.use-case.ts:20` | Contradicts the live route; misleads every future reader |
| `hasByokKey` per-provider instead of aggregate     | `api/manifest/capabilities/route.ts:22-26`      | Known defect, user-visible lie in the capabilities UI    |
...
> **GATE (tightened per independent review Part 2):** at least two external parties running the check in CI unprompted a month after install — **or** one engagement where the client asks to keep the tooling (pays for it, or requests the handoff pack) after the engagement ends. The previous "visibly changed the client conversation" escape hatch is removed as self-judged and satisfiable by charitable interpretation.
...
| Server-side project/run persistence | 2–3 wks        | Pattern exists (better-sqlite3 + Docker volume for quota/BYOK). Replace the 501 stub at `api/projects/[projectId]`; server becomes authoritative, IDB becomes cache. Honor D-4 before writing schema. |
