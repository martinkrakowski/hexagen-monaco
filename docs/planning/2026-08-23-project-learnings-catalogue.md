...
- **Thirteen supersession pairs exist in the ADR texts (2 full, 7 partial, 4 amendments); before 2026-08-23 only two superseded ADRs said so** (ADR-0038 partially, ADR-0049 — both via the inline `**Status:**` form). The first draft of this bullet got three things wrong, each a lesson on its own: it said _twenty-six_ — a count of ADRs that _mention_ an earlier one, not of supersessions, and no pair list backed it; it said ADR-0007 and ADR-0034 carry no status line — both do (`**Status:** Accepted` / `**Status**: ACCEPTED`), the grep matched one of the two status formats (`## Status` block vs inline `**Status:**`) and the other format's variant spelling; and it said ADR-0022 narrows ADR-0021 — ADR-0022 does not mention 0021 at all. All three were caught when the Phase 1 worker derived the pairs from the texts rather than from this document (#629). Every pair is now marked on both sides. The reusable lesson: **a sweep that greps for one spelling of a marker is not a sweep; and a count without a list behind it is a guess wearing a number.**
