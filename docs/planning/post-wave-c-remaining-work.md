...
| D13 | Generator workflow templates pin `actions/*@v4`        | **#323 review find.** Shipped `ci-github-actions`/`docker`/`bedrock-agentcore-runtime` template families + `project-generation/src/assets/workflow-template.yml` pin `actions/{checkout,setup-node,cache}@v4` + Node 20 → generated projects inherit the June-16 Node-24 deprecation (warnings first, eventual breakage). Bump the **sources** to `@v5`/Node ≥22 (`dist` regenerates). The repo's own `.github/workflows/` are already `@v5`. |
