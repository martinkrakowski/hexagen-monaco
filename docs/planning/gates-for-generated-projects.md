...
**Corrected 2026-09-17, before a lane was dispatched.** The first version of this finding — and of
`tools/arch-linter/findings/0001` — described applications as _contexts whose root does not exist_, so the
linter _skipped_ them. Both halves fail on one fact: **an application is not a context.**

```
cli.ts:623   const modules = ... = manifest.bounded_contexts ?? []   ← the only scan set
grep -rnE '\.apps\b|app\.yaml' tools/arch-linter/src → no matches   ← apps are never read
.architecture/manifest.yaml:36 bounded_contexts:   :193 apps:        ← separate sections
```

Applications are not skipped members of the iterated set — they are **outside the set that is iterated**.
No missing root and no `abortIfVacuous` is involved, because no `apps/` entry ever reaches the loop that
would check for one. The consequence is unchanged and if anything sharper: layer rules do not apply to
`apps/` **by construction**, and a green `lint:arch` says nothing about them.
