...
- **Risk:** the gate softening (B3) trades absolute count-preservation for "no net deletion + no context drift." Lower than it reads — gaming-by-padding is already caught by the accept criterion (a filler port raises `portCount` but doesn't lower `errorsAfter`, so `errorsAfter < errorsBefore` fails). The only genuinely residual case is a same-error-count restructuring inside the no-shrink envelope — also rejected, since `errorsAfter` would not be strictly lower. Revisit only if a real regression surfaces.
