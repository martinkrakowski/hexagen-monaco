...
| **G4**  | **A query API over the installed store.** `listFindings({ template?, version?, status? })` resolving against `dist/templates` in a published package and `../template-engine/templates` in the monorepo — the same dual resolution `resolveTemplatesDir()` already does. Pure, offline, no network.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `packages/template-findings`, tests                                          | The read path                                             |
...
- **G4**: resolves in both layouts (monorepo and installed package); filtering by version excludes
  a finding fixed before the version asked for; no network call is possible (assert the module
  imports nothing that can make one).
