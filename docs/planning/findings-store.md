...
- **G4** (rewritten 2026-09-09 — the old text asked the module to resolve a directory it cannot
  reach): the **reader is infrastructure** and the **filtering is domain**; `listFindings` takes a
  templates directory and reads only beneath it,
...
- **Publishing `@hexagen/template-engine` as its own npm package** — **considered and declined,
  2026-09-09.** It solves nothing here and breaks the join key. Nothing is currently unreachable:
  `@hexagen-monaco/sync` publishes a library surface (`exports: "."` → `dist/index.js` + types)
  beside its `hexagen` bin, so G5 and G6 can both import the query API from it today.
  The cost is concrete: `template-engine`'s `files` is `["dist", "templates"]` — **2.0 MB of
  templates** — and tsup already copies that same tree into `packages/sync/dist/templates`
  (ADR-0068, `noExternal: [/^@hexagen\//]`). Publishing both ships the payload **twice, in two
  independently-versioned tarballs**, so a project could resolve a finding from one copy and the
  template it describes from the other. F-D6 makes the join key `(subject id, subject version)`
  and F-D2 rests on the payload being pinned to the version the project runs; two pins that can
  diverge is **a second source of truth for the same fact** — the mistake F-D7 already made once
  in this document, and the defect class this store exists to collect.
  ADR-0068 (2026-04-25) decided publish-one-package over publish-the-workspace-deps on
  transitive-version-mismatch grounds, before findings existed to sharpen it. One nuance that
  _weakens_ the ADR here and should be recorded honestly: its largest objection was a ~900-file
  `.js`-extension migration for nodeNext, and that does not apply to this package —
  `template-engine` already writes extensions on 107 imports with zero extensionless. It is
  technically publishable; the version-coupling objection is the one that still stands.
  **If the need ever appears**, the escape hatch is the one §5 already names — a _data-only_
  findings package with no code to drift and a deliberate second version axis — not publishing
  all of `template-engine`. Do not build it speculatively.
