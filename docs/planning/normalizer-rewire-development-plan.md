...
3. **Quantitative rollback triggers (defined here per Q1):** any failed gate ⇒ set `STAGED_GENERATION_PIPELINE=stub` and investigate before resuming the canary.
   ...
   - **T2 — latency:** full p95 (successful runs) must not exceed **max(2× stub p95, 45s absolute ceiling)** (`T2_ABSOLUTE_CEILING_MS`). _Recalibrated 2026-06-10:_ the original relative-only bound stops discriminating when the stub baseline is fast — the five-model sweep had claude-3.5-haiku fail at 4.47× while genuinely slow (99.7s p95) and mercury-2 fail at 4.2× while the fastest pipeline ever measured (11.8s p95). The ceiling is anchored just above the post-#292 gpt-4o full p95 (42.9s); the relative bound still governs slow-stub regimes, and the degenerate 0ms-stub case is now governed by the ceiling instead of being unevaluable.
...
- **Weak-model hardening (found 2026-06-10, second A3 baseline on `gpt-4o-mini` — gates green, output degenerate):** two failure modes the gates structurally miss. (a) **Stage-1 under-decomposition:** domain analysis returned a single subdomain (`['E-commerce']`) for the 8-concern ecommerce golden prompt → 1-context manifests; stage 2 rejects nothing, T4 passes (non-zero), so the collapse is invisible. Consider a stage-1 decomposition floor or escalation. (b) **Stage-6 judge exemplar parroting:** the judge copied the prompt's `postgres-repo` few-shot — `R01: contains technology noun 'Postgres'` reported for every context in every run (incl. names like `book-collection-manager`), plus fabricated "marked as shared-kernel" claims; both pipelines judge at 0% and T3's rate clause turns vacuous. Fix: compute R01 deterministically in Stage 6 (`isBannedContextName` over accepted names, injected into the report instead of asked of the LLM); consider the same for the structurally checkable rules (R02–R09).
