...
46 severities lowered including all three criticals; 10 recommendations unsound;
...
- **One item = one worktree = one PR.** Parallel items get parallel worktrees
  (`isolation: worktree`); nothing shares a checkout. Worktrees lack
  `node_modules` — run gates from the main checkout or install first.
...
| 5.3 | HEX-012, HEX-020 (residue) | Catalog out of domain per ADR 0.5 — split: (a) local-llm, (b) agentic-interaction (`createDefaultFallbackChain` deleted once consumers move), (c) wire.client's inline `ProjectDiscarded` purge cascade extracted into the existing `discardProject` use case. The env-derived chain **stays** in wire.server | ADR 0.5              |
...
| 6.4 | HEX-018          | ports/in → ports/out moves, **one package per PR**: governance, monaco-orchestration, wizard-orchestration, project-configuration (+ security if kept)                                                                                                                                                                                                                         | ADR 0.2                               |
| 6.5 | HEX-019          | mcp-server tools onto inbound ports, split by tool family (~3 PRs, not one 25-class PR)                                                                                                                                                                                                                                                                                        | 6.4, ADR 0.2 supersession of ADR-0010 |
...
| 8.12 | HEX-024, HEX-027, HEX-028, HEX-029, HEX-032, GOD-008 (residue), GOD-011, AUD-017 | Leftovers, one small PR each: model-prefs port; template-engine path predicates; local-llm port wording; prompt-compiler type ownership; intent-compiler invariants; **name** the scaffold-defaults literal (the rest of GOD-008's rec is already satisfied — audited); IDB salvage mapper; move the Generate-with-AI screen-flow state machine + device copy out of manifest-generation domain into the web feature / model-selection layer | GOD-008 after 5.2                                                               |
...
2.2 and 3.4 and 4.7 change the published packages — they ride the next npm release
(explicit go-ahead required, per standing policy). Everything else is repo-internal.
