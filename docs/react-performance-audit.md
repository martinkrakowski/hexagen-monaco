...
| **7. Correctness: Side Effects in `setState` Updater** | 2 | 2 | 0 | 0 |
...
| **P0-C** | **Historical Context: Fix C2** | C2 | Prevents stale graph after wizard edits | **Historical Context:** `wizardData` was kept in deps during triage. 9-commit canvas cascade per keystroke is correct behavior (graph must update); debatching is a separate optimization. | **No stale bug in committed code** — `wizardData` was kept in deps during triage. |
